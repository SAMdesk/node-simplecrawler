...
FetchQueue.prototype.freezeSync = function() {
    var queue = this;
...
FetchQueue.prototype.defrostSync = function(file, callback) {
    var queue = this,
        defrostedQueue = [];
        
    if (!file.toString("utf8").length) {
        throw new Error("Failed to defrost queue from zero-length JSON.");
    }

    try {
        defrostedQueue = JSON.parse(file.toString("utf8"));
    } catch (error) {
        throw error;
    }
