...
Crawler.prototype.fetchQueueItem = function(queueItem) {
    ...
    crawler.queue.update(queueItem.id, {
        ...
        try {
            clientRequest = client.request(requestOptions, function(response) {
                crawler.handleResponse(queueItem, response, timeCommenced);
            });
        } catch (e) {
            // todo remove this if the event emitted below works correctly
            var _url = requestOptions.host;
            
            if(!!requestOptions.port) _url += ':' + requestOptions.port;
            
            _url += requestOptions.path;
            
            console.log('Exception thrown while requesting ' + _url + ': ' + JSON.stringify(e));
             ...
             */
            crawler.emit("fetchclienterror", queueItem, e);
            ...
            crawler.queue.update(queueItem.id, {
                ...
                if (error) {
                    crawler.emit("queueerror", error, queueItem);
                }
            });
